...
import { BigPlayButton, Player,ControlBar,ForwardControl,
       ReplayControl ,PlaybackRateMenuButton} from "video-react"
...
const VideoDetails = () => {
  ...
  // check 1st lecture video of the course
  const isFirstVideo = () => {
  ...
  // check last lecture video of the course
  const isLastVideo = () => {
    const currentSectionIndx = courseSectionData.findIndex(
      (data) => data._id === sectionId
    )
      ...
      {!videoData ? ( <img src={previewSource} alt="Preview" className="h-full w-full rounded-md object-cover"/> ) : (
       ...
       <div className=" my-[4%] mx-[7%] w-[83%] shadow-[10px_-9px_15px_-9px] shadow-richblack-800">
        ...
        <ControlBar >
           ...
           <PlaybackRateMenuButton rates={[0.75, 1, 1.25, 1.5, 1.75, 2]} order={7.1} />
       </ControlBar>
              ...
              <IconBtn
                ...
                onclick={() => {
                  if (playerRef?.current) {
                    // {console.log(playerRef.current.seek,"xxggaa")}
